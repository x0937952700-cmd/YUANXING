...
def orders():
    c=conn();cur=c.cursor()
    cur.execute("SELECT action FROM logs WHERE action LIKE '出貨%'")
    d=cur.fetchall()
    c.close()
    return {"items":[{"product":i[0]} for i in d]}

# 倉庫
@app.route("/api/w")
def w():
    c=conn();cur=c.cursor()
    cur.execute("SELECT product,location FROM inventory")
    d=cur.fetchall()
    c.close()
    return jsonify(d)
...
def act():
    c=conn();cur=c.cursor()
    cur.execute("SELECT action FROM logs ORDER BY id DESC LIMIT 20")
    d=cur.fetchall()
    c.close()
    return {"items":[{"action":i[0]} for i in d]}
