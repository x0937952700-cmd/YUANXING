...
from db import (
    ...
    get_activity_logs, get_today_shipping_qty, rows_to_dict, get_db, sql, now,
    ensure_runtime_schema, warehouse_add_cell, warehouse_delete_cell,
)
...
def protect_pages():
    path = request.path
    if path.startswith("/static/") or path in ("/health", "/manifest.json", "/manifest.webmanifest", "/sw.js", "/service-worker.js"):
        return None
    public = ["/login", "/api/login", "/health"]
    if path in public:
        return None
...
def home():
    if not require_login():
        return redirect(url_for("login_page"))
    return render_template("index.html", username=current_username(), title="沅興木業", today=now()[:10])
...
@app.route("/activity")
def activity_page():
    return render_template("module.html", module_key="activity", title="今日異動", username=current_username())
...
def api_upload_ocr():
    try:
        ...
        cropped_text = (request.form.get("ocr_text") or "").strip()
        if not file:
        ...
        log_action(current_username(), f"OCR辨識｜信心值:{confidence}%｜引擎:{result.get('engine','')} ")
        warning = result.get("warning") or ("辨識信心偏低，請確認內容" if confidence < 80 else "")
        return jsonify(
            success=True,
            text=result.get("text", "") or cropped_text,
        ...
        return jsonify(success=True, text="", items=[], confidence=0, warning="辨識失敗，請手動編輯", hints=["請確認已設定 OCR_SPACE_API_KEY"], engine="error")
    ...
    if items:
        ...
        for it in items:
            ...
            if qty <= 0:
                continue
            cleaned.append({
                "product_text": it.get("product_text") or it.get("product") or it.get("product_name") or "",
...
def api_inventory():
    try:
        ...
        log_action(operator, f"建立庫存｜客戶:{customer_name or '未指定'}｜位置:{location or '未指定'}｜商品:{'、'.join([it.get('product_text', '') for it in items[:3]])}")
        return jsonify(success=True, items=grouped_inventory())
    except Exception as e:
...
def api_orders():
    try:
        ...
        log_action(current_username(), f"建立訂單｜客戶:{customer_name}｜商品:{'、'.join([it.get('product_text', '') for it in items[:3]])}")
        return jsonify(success=True, items=get_orders())
    except Exception as e:
...
def api_master_orders():
    try:
        ...
        data = request.get_json(silent=True) or {}
        items = _parse_items_from_request(data)
        customer_name = (data.get("customer_name") or "").strip() or "未指定"
        save_master_order(customer_name, items, current_username())
        log_action(current_username(), f"更新總單｜客戶:{customer_name}｜商品:{'、'.join([it.get('product_text', '') for it in items[:3]])}")
        rows = get_master_orders()
        for r in rows:
            r["customer_name"] = ""
        return jsonify(success=True, items=rows)
...
def api_ship():
    try:
        ...
        if result.get("success"):
            log_action(current_username(), f"完成出貨｜客戶:{customer_name}｜商品:{'、'.join([it.get('product_text', '') for it in items[:3]])}")
...
def api_customers():
    try:
        ...
        log_action(current_username(), f"儲存客戶｜{name}｜區域:{data.get('region') or '北區'}")
        return jsonify(success=True, items=get_customers())
    except Exception as e:
...
def api_warehouse_move():
    try:
        ...
        if result.get("success"):
            log_action(current_username(), f"拖曳商品｜{product_text}｜{from_key[0]}{from_key[1]}-{from_key[2]}-{from_key[3]}→{to_key[0]}{to_key[1]}-{to_key[2]}-{to_key[3]}")
        return jsonify(result)
    except Exception as e:
...
@app.route("/api/warehouse/add-cell", methods=["POST"])
@login_required_json
def api_warehouse_add_cell():
    try:
        data = request.get_json(silent=True) or {}
        warehouse_add_cell(
            data.get("zone"), int(data.get("column_index")), data.get("slot_type"), int(data.get("slot_number"))
        )
        log_action(current_username(), "新增倉庫格位")
        return jsonify(success=True, zones=warehouse_summary())
    except Exception as e:
        log_error("warehouse_add_cell", str(e))
        return error_response("新增格位失敗")


@app.route("/api/warehouse/delete-cell", methods=["POST"])
@login_required_json
def api_warehouse_delete_cell():
    try:
        data = request.get_json(silent=True) or {}
        ok = warehouse_delete_cell(
            data.get("zone"), int(data.get("column_index")), data.get("slot_type"), int(data.get("slot_number"))
        )
        if not ok:
            return error_response("格位內仍有商品，不能刪除")
        log_action(current_username(), "刪除倉庫格位")
        return jsonify(success=True, zones=warehouse_summary())
    except Exception as e:
        log_error("warehouse_delete_cell", str(e))
        return error_response("刪除格位失敗")
...
def _today_activity_summary(seen_after=None):
    logs = get_activity_logs(limit=300, today_only=True)
    inventory_rows = inventory_summary()
    unplaced_qty = sum(int(r.get('unplaced_qty', 0)) for r in inventory_rows)
    shipping_qty = get_today_shipping_qty()
    log_additions = sum(1 for r in logs if str(r.get('action', '')).startswith(('建立', '儲存', '更新')))
    latest = logs[0]['created_at'] if logs else ''
    unread = sum(1 for r in logs if str(r.get('created_at', '')) > seen_after) if seen_after else len(logs)
    return {
        'today_new': int(log_additions),
        'today_shipping_qty': int(shipping_qty),
        'unplaced_qty': int(unplaced_qty),
        'unread': int(unread),
        'latest': latest,
    }
...
def api_activity_summary():
    seen_after = (request.args.get('seen_after') or '').strip() or None
    return jsonify(success=True, summary=_today_activity_summary(seen_after))
...
def api_activity_feed():
    ...
    logs = get_activity_logs(limit=limit, today_only=True)
    feed = []
    for row in logs:
        feed.append({
            'id': row.get('id'),
    ...
    return jsonify(success=True, items=feed[:limit], summary=_today_activity_summary())
...
def api_activity_delete():
    ...
    try:
        ...
        conn.rollback()
        log_error('activity_delete', str(e))
