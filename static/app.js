...
async function pollActivityStatus(showToastOnNew=true){
  try {
    ...
    const homeCount = $('home-activity-count');
    if (homeCount) homeCount.textContent = String(summary.unread || 0);
...
function attachSwipeDelete(row, id){
  ...
  row.addEventListener('touchmove', (e) => {
    ...
    if (dx < 0) {
      e.preventDefault();
      row.style.transform = `translateX(${Math.max(dx, -180)}px)`;
    }
  }, { passive: false });
...
function renderWarehouseZoneFrame(zone){
  ...
  if (!frame) return;
  frame.innerHTML = '';
  const q = (state.warehouse.warehouseSearchQuery || '').trim().toLowerCase();
  for (let band = 1; band <= 6; band++) {
    ...
    ['front', 'back'].forEach(side => {
      ...
      for (let n = 1; n <= 10; n++) {
        ...
        const cellText = items.map(it => `${it.product_text || it.product || ''} ${it.customer_name || ''}`).join(' ').toLowerCase();
...
async function clearWarehouseSearch(){
  if ($('warehouse-search')) $('warehouse-search').value = '';
  state.warehouse.warehouseSearchQuery = '';
  renderWarehouse();
}

function debouncedWarehouseSearch(){
  clearTimeout(window.__warehouseSearchTimer);
  window.__warehouseSearchTimer = setTimeout(() => searchWarehouse(), 200);
}

async function searchWarehouse(){
  const q = ($('warehouse-search')?.value || '').trim();
  state.warehouse.warehouseSearchQuery = q;
  if (!q) {
    await renderWarehouse();
    return;
  }
...
function clearCustomerSearch(){
  if ($('customer-search')) $('customer-search').value = '';
...
function debouncedCustomerSearch(){
  ...
  window.__customerSearchTimer = setTimeout(() => renderCustomers(), 160);
}
...
s.src = "/integration/frontend_all.js";
