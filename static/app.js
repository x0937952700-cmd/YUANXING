(() => {
  ...
  function renderInventoryList(items) {
    const warehouseCells = window.__warehouseCells || [];
    const isPlaced = (product) => warehouseCells.some(c => {
      const p = String(c.product || '').trim();
      const n = String(c.customer_name || '').trim();
      return product && (p === product || n === product || p.includes(product) || product.includes(p));
    });
    setResult(`
      <div class="table-wrap">
        <table class="data-table">
          ...
          <tbody>
            ${items.map(i => {
              const placed = isPlaced(String(i.product || '').trim());
              return `<tr class="${placed ? '' : 'item-unplaced'}"><td>${escapeHTML(i.product)}</td><td>${escapeHTML(i.quantity)}</td><td>${escapeHTML(i.location || '')}</td><td>${escapeHTML(i.operator || '')}</td><td>${escapeHTML(i.updated_at || '')}</td></tr>`;
    ...
    setResult(`
      <div class="table-wrap">
        <table class="data-table">
          ...
          <tbody>
            ${items.map(i => `<tr class="${isPlaced(String(i.product || '').trim()) ? '' : 'item-unplaced'}"><td>${escapeHTML(i.customer)}</td><td>${escapeHTML(i.product)}</td><td>${escapeHTML(i.qty)}</td><td>${escapeHTML(i.status)}</td><td>${escapeHTML(i.operator || '')}</td><td>${escapeHTML(i.created_at || '')}</td></tr>`).join('') || '<tr><td colspan="6">尚無資料</td></tr>'}
          </tbody>
        </table>
      </div>
    `);
    ...
    setResult(`
      <div class="table-wrap">
        <table class="data-table">
          ...
          <tbody>
            ${items.map(i => `<tr class="${isPlaced(String(i.product || '').trim()) ? '' : 'item-unplaced'}"><td>${escapeHTML(i.customer)}</td><td>${escapeHTML(i.product)}</td><td>${escapeHTML(i.qty)}</td><td>${escapeHTML(i.operator || '')}</td><td>${escapeHTML(i.updated_at || '')}</td></tr>`).join('') || '<tr><td colspan="5">尚無資料</td></tr>'}
          </tbody>
        </table>
      </div>
    `);
  ...
  async function initModulePage() {
    ...
    if (['inventory','order','master_order','ship'].includes(page)) {
      ...
      const cameraInput = qs('#camera-input');
      const cameraBtn = qs('#camera-btn');
      ...
      cameraInput?.addEventListener('change', async () => {
        await uploadOCR(cameraInput, statusEl, confEl);
        cameraInput.value = '';
      });
      ...
      if (['inventory','order','master_order','ship'].includes(page)) {
        const warehouseData = await apiJSON('/api/warehouse');
        window.__warehouseCells = warehouseData?.cells || [];
      }
    ...
    if (page === 'warehouse') {
      ...
      const searchInput = qs('#warehouse-search');
      let currentCells = [];
      ...
      async function loadAndRender() {
        ...
        inventoryProducts = Array.from(new Set([...(inv || []).map(i => String(i.product || '').trim()), ...(cells || []).map(c => String(c.product || '').trim()), ...(cells || []).map(c => String(c.customer_name || '').trim())].filter(Boolean)));
        window.__warehouseCells = cells;
        window.__warehouseProductOptions = inventoryProducts;
        ...
        const matchCell = (cell) => {
          if (!term) return true;
          const joined = [cell.label, cell.customer_name, cell.product, String(cell.quantity || ''), `${cell.area}${cell.col_no}${cell.position}`].join(' ').toLowerCase();
        ...
        grid.innerHTML = ['A', 'B'].map(area => `
          <section class="warehouse-area">
            ...
            <table class="warehouse-table">
              <thead>
                <tr>
                  ${Array.from({length: 12}, (_, idx) => `<th>${idx + 1} 欄</th>`).join('')}
                </tr>
              </thead>
              <tbody>
                <tr>
                  ${Array.from({length: 12}, (_, idx) => {
                    ...
                    return `<td>${matchCell(front) ? `<div class="warehouse-cell ${front.product ? '' : 'empty'}" draggable="true" data-area="${area}" data-col="${col}" data-position="front">
                        <div><small>前</small><br><strong>${escapeHTML(front.label || `${area}${col} 前`)}</strong></div>
                        <div class="qty ${front.customer_name ? '' : 'muted'}">${escapeHTML(front.customer_name || '尚未設定')}</div>
                        <div class="qty">${escapeHTML(front.product || '')}</div>
                        <div class="qty">庫存：${escapeHTML(front.quantity || 0)}</div>
                      </div>` : '<div class="warehouse-cell empty muted"><small>不符合搜尋</small></div>'}</td>`;
                  }).join('')}
                </tr>
                <tr>
                  ${Array.from({length: 12}, (_, idx) => {
                    ...
                    return `<td>${matchCell(back) ? `<div class="warehouse-cell ${back.product ? '' : 'empty'}" draggable="true" data-area="${area}" data-col="${col}" data-position="back">
                        <div><small>後</small><br><strong>${escapeHTML(back.label || `${area}${col} 後`)}</strong></div>
                        <div class="qty ${back.customer_name ? '' : 'muted'}">${escapeHTML(back.customer_name || '尚未設定')}</div>
        ...
        let dragSource = null;
        qsa('.warehouse-cell[draggable="true"]', grid).forEach(cell => {
          ...
          cell.addEventListener('dragstart', e => {
            dragSource = { area: cell.dataset.area, col: Number(cell.dataset.col), position: cell.dataset.position };
          ...
          cell.addEventListener('drop', async e => {
            ...
            const tgt = { area: cell.dataset.area, col: Number(cell.dataset.col), position: cell.dataset.position };
            if (src.area === tgt.area && src.col === tgt.col && src.position === tgt.position) return;
            const sourceCell = cells.find(x => x.area === src.area && Number(x.col_no) === Number(src.col) && x.position === src.position);
            const targetCell = cells.find(x => x.area === tgt.area && Number(x.col_no) === Number(tgt.col) && x.position === tgt.position);
            if (!sourceCell) return;
            const a = { ...sourceCell, area: tgt.area, col_no: tgt.col, position: tgt.position };
            const b = sourceCell && targetCell ? { ...targetCell, area: src.area, col_no: src.col, position: src.position } : null;
            await apiJSON('/api/warehouse', { method: 'POST', body: JSON.stringify(a) });
            if (b) await apiJSON('/api/warehouse', { method: 'POST', body: JSON.stringify(b) });
            toast('格位已移動');
            await loadAndRender();
          });
        });
      ...
      function openWarehouseCellModal(cells, area, col, position, createNew = false) {
        ...
        const options = Array.from(new Set([...(window.__warehouseProductOptions || []), ...cells.map(x => x.product), ...cells.map(x => x.customer_name)].filter(Boolean)));
        const html = `
          <div class="modal-body-grid single">
            <div class="form-row">
              <input id="w-area" class="text-input" value="${escapeHTML(existing.area)}" placeholder="A/B" />
              <input id="w-col" class="text-input" type="number" value="${escapeHTML(existing.col_no)}" min="1" max="12" />
            </div>
            <div class="form-row">
            ...
            <div class="form-row">
              <input id="w-search" class="text-input" value="" placeholder="搜尋已錄入商品 / 客戶（例：113）" />
              <select id="w-select" class="text-input select-input">
                <option value="">下拉選擇已錄入項目</option>
        ...
        const searchEl = qs('#w-search');
        const selectEl = qs('#w-select');
        const productEl = qs('#w-product');
        const customerEl = qs('#w-customer');
        const filteredOptions = () => {
          const term = String(searchEl?.value || '').trim().toLowerCase();
          const select = qs('#w-select');
          if (!select) return;
          const opts = options.filter(opt => !term || String(opt).toLowerCase().includes(term));
          select.innerHTML = `<option value="">下拉選擇已錄入項目</option>` + opts.map(opt => `<option value="${escapeHTML(opt)}">${escapeHTML(opt)}</option>`).join('');
        };
        searchEl?.addEventListener('input', filteredOptions);
        selectEl?.addEventListener('change', () => {
          const value = selectEl.value || '';
          if (!value) return;
          productEl.value = value;
          customerEl.value = customerEl.value || value;
        });
        filteredOptions();
